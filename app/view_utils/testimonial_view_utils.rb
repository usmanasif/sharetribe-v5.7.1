module TestimonialViewUtils
  ...
  def received_neutral_testimonials_in_community(person, community)
    person.received_neutral_testimonials.includes(:tx).select{|t|t.tx.community_id == community.id }
  end
end
