...
class Person < ApplicationRecord
  ...
  has_many :received_positive_testimonials, -> { where("grade IN (0.75,1)").order("id DESC") }, :class_name => "Testimonial", :foreign_key => "receiver_id"
  has_many :received_neutral_testimonials, -> { where("grade IN (0.5)").order("id DESC") }, :class_name => "Testimonial", :foreign_key => "receiver_id"
